...
def searchForEachKW(KW, script, listKWs, listKWsDate):
    ...
    print(type(script),"script type in searchForEachKW")
    print('listKWsDate',listKWsDate)
    print('listKWs',listKWs)
    ...
    if KW in script:
        ...
        # shortcode includes forward slashes around

        dateIdx = script.find("created_at") + 12
        #print('dateIdx', dateIdx)
        unixDate = script[dateIdx:dateIdx + 10]
        #print('unixDate', unixDate)
        regularDate = dt.datetime.fromtimestamp(int(unixDate)).strftime(
            '%Y-%m-%d ''%H:%M:%S')
        print('regularDate',regularDate)
        #will format like 2017-12-14 09:17:06

        i=0
        while i<len(listKWs):
            if listKWs[i][0]==KW:
                listKWs[i].append(insta_link)
                listKWsDate[i].append(regularDate)
            i+=1
        ##by end should have list with url and date added to appropriate
        # keyword sublists
        # time.sleep(10)
...
if __name__ == '__main__':
    ...
    for k in listOfKWs:
        print (k,'index',listOfKWs.index(k))
